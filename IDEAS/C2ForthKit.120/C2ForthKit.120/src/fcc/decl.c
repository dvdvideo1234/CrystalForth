...
19980713 MSD001 Correct a bug in FIELDS().  The call to NEWFIELD() relied upon
                the compiler evaluating the third parameter to NEWFIELD() (the
                call to DCLR()) before the first parameter (the value of
                ID). The address of ID was passed to DCLR() to be filled in
                before being passed to NEWFIELD().  This worked under Watcom
                and Borland but not under Microsoft.  I suppose that
                assumption was that since most 80x86 C compilers push
                parameters right-to-left, that they must *evaluate* them in
                this order...
...
static Type specifier(sclass) int *sclass; {
	...
	if (vol  == VOLATILE)
        ...
        /*PLC: Error if double, float, or long double used.
        */
        if (unqual(ty) == floattype ||
            unqual(ty) == doubletype ||
            unqual(ty) == longdouble)
...
char *id; Type ty; Coordinate *pos; {
	Symbol p, q;
...
static void initglobal(p, flag) Symbol p; {
	Type ty;
...
Type basety; char **id; Symbol **params; {
	...
	{ int n = 0;
	  while (istypename(t, tsym)) {
	  	...
	  	for (;;) {
	  		Field p, temp;                                          /* MSD001 */
	  		char *id = NULL;
                        temp = dclr(ty1, &id, NULL, 0);                         /* MSD001 */
	  		p = newfield(id, ty, temp);                             /* MSD001 */
			  ...
			  else if (id == NULL && isstruct(p->type)) {
			  	if (Aflag >= 2)
...
static void funcdefn(sclass, id, ty, params, pt)
...
int loop, lev; struct swtch *swp; {
	...
	{
		int i;
		Symbol *a = xltoa(&autos, STMT);
		nregs = length(registers);
		for (i = 0; a[i]; i++)
			registers = append(a[i], registers);
		cp->u.block.locals = xltoa(&registers, FUNC);
	}
...
Type enumdcl() {
	...
	if (t == '{') {
		...
		ty->u.sym->u.idlist = xltoa(&idlist, PERM);
		ty->u.sym->defined = 1;
