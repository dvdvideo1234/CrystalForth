...
static int xdiv ARGS((double, double, double, double, int));
...
Tree simplify(op, ty, l, r) int op; Type ty; Tree l, r; {
	...
	switch (op) {
		...
		case CVI+C: xcvtcnst(I, chartype,l->u.v.i,SCHAR_MIN,SCHAR_MAX,
				p->u.v.sc = l->u.v.i); break;
		case CVD+F: xcvtcnst(D,floattype,l->u.v.d, -FLT_MAX,FLT_MAX,
				p->u.v.f  = l->u.v.d); break;
		case CVD+I: xcvtcnst(D,  inttype,l->u.v.d,  INT_MIN,INT_MAX,
				p->u.v.i  = l->u.v.d); break;
		case CVI+S: xcvtcnst(I,shorttype,l->u.v.i, SHRT_MIN,SHRT_MAX,
				p->u.v.ss = l->u.v.i); break;

/* PLC - TS C can't handle this. */
		...
		case BXOR+U:
			...
			break;
		case DIV+D:
			xfoldcnst(D,d,/,doubletype,xdiv,-DBL_MAX,DBL_MAX,0);
			break;
		case DIV+F:
			xfoldcnst(F,f,/,floattype,xdiv,-FLT_MAX,FLT_MAX,0);
			break;
		case DIV+I:
			identity(r,l,I,i,1);
#ifdef mips
			if (l->op == CNST+I && r->op == CNST+I && r->u.v.i == -1
			&& !xdiv((double)l->u.v.i, (double)r->u.v.i, (double)INT_MIN, (double)INT_MAX, 0))
				break;
#endif
			xfoldcnst(I,i,/,inttype,xdiv,INT_MIN,INT_MAX,needconst);
			break;
		case DIV+U:		
			identity(r,l,U,u,1);
			if (r->op == CNST+U && r->u.v.u == 0)
...
/* xdiv - return 1 if min <= x/y <= max, 0 otherwise */
static int xdiv(x, y, min, max, needconst) double x, y, min, max; {
	int cond;
