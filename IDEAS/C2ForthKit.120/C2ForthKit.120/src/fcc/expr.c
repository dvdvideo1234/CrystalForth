...
Type binary(xty, yty) Type xty, yty; {
	...
	if (isunsigned(xty) || isunsigned(yty))
		return unsignedtype;
/*PLC: begin */
        /* Unsigned short gets promoted to unsigned if
           sizeof(unsigned short) == sizeof(int) */
	if ((unqual(xty) == unsignedshort || unqual(yty) == unsignedshort) &&
	    unsignedshort->size == inttype->size)
	  return unsignedtype;
/*PLC: end */
	return inttype;
}
...
Tree cast(p, type) Tree p; Type type; {
	...
	if (isstruct(pty) && isstruct(ty) && extends(pty, ty)) {
	...
	case POINTER:
		if (isptr(ty)) {
			Field q;
			if (isstruct(pty->type) && isstruct(ty->type)
