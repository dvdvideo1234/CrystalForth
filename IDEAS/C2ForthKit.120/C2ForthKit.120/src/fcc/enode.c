...
Tree f, args; Type ty; Symbol t3; {
	...
	if (isstruct(ty))
		...
		if (isenum(ty))
			...
			rty = unsignedtype;
		p = tree(CALL + widen(rty), rty, f, NULL);
		if (isptr(ty))
			p = cast(p, ty);
	}
	return p;
...
/* Added by PLC for more relaxed pointer checking. */		
		else
		{
			warning("pointer conversion between `%t' and `%t'\n",
				xty, yty);
			return xty;
		}
/* End of addition. */		
...
static Tree andtree(op, l, r) Tree l, r; {
...
static Tree multree(op, l, r) Tree l, r; {
	Type ty = inttype;
...
static Tree subtree(op, l, r) Tree l, r; {
	int n;
	Type ty = inttype;
