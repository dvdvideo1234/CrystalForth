...
static int haskid ARGS((Node, Node));
...
/*PLC; don't CSE consant nodes.
		      	p = node(tp->op, NULL, NULL, constant(ty, tp->u.v)); } break;
*/	      	
		      	p = newnode(tp->op, NULL, NULL, constant(ty, tp->u.v)); } break;
	case RIGHT: { if (   tp->kids[0] && tp->kids[1]
			      ...
			      if (0) {
			      	...
			      	if (tp->kids[0]->op == FIELD)
			      		q = tp->kids[0]->kids[0]->node;
			      	assert(q);
			      	for (t = forest->link; t != q; t = t->link)
			      		if (haskid(q, t))
			      			assert(0);
			      }
#endif
...
#undef GLOBAL_CSE
#ifdef GLOBAL_CSE
/* PLC: CSE addresses of globals */
		else if (p->op == ADDRLP || p->op == ADDRFP) {
#else		
/* PLC: don't CSE addresses of globals */
		else if (p->op == ADDRLP || p->op == ADDRFP || p->op == ADDRGP) {
#endif
			assert(!listed);
